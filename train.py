...
########### GET DATA #############
fileList = list(filter(bool,[line.rstrip('\n') for line in open('config_files/config_files.txt')]))

noSyls =  list(map(int, list(filter(bool,[line.rstrip('\n') for line in open('config_files/config_sylls.txt')]))))
...
ids = tf.keras.Input(shape=(None, 2), dtype=tf.int32)
ids_len = tf.keras.Input(shape=(None,), dtype=tf.int32)
is_train = tf.keras.Input(shape=(), dtype=tf.bool)
...
x = tf.keras.Input(shape=(None, input_channels), dtype=tf.float32)
y = tf.keras.Input(shape=(None,), dtype=tf.float32)
y_c = tf.keras.Input(shape=(None, maxT), dtype=tf.float32)
